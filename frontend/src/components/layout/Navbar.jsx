...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 z-10 flex h-[5rem] w-full items-center justify-between bg-highlight px-8 py-2 dark:bg-gradient-to-b dark:from-highlight dark:to-shadow2">
      ...
      {/* 桌面版導航選項 */}
      <div className="hidden h-[4rem] w-[35.88rem] items-end justify-around font-sf text-xxs font-medium md:flex">
        {navItems.map((item) => (
          <button
            key={item.name}
      ...
      {/* 右側語言與模式切換（桌面版始終顯示） */}
      <div className="hidden h-[4rem] w-[35.88rem] items-start justify-end gap-[1rem] md:flex">
